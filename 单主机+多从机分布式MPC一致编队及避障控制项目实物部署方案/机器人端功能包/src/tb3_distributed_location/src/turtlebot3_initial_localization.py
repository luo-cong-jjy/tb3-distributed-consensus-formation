...
def set_initial_pose():
    global namespace, pose_params
    # 使用命令行传入的初始位姿参数（效率最高，不占用ROS参数服务器）
    pose = {"ns": namespace, "x": pose_params["x"], "y": pose_params["y"], "yaw": pose_params["yaw"]}
...
if __name__ == '__main__':
    try:
        if len(sys.argv) < 5:
            rospy.logerr("Usage: turtlebot3_initial_localization.py <namespace> <x> <y> <yaw>")
            sys.exit(1)

        namespace = sys.argv[1]
        initial_x = float(sys.argv[2])
        initial_y = float(sys.argv[3])
        initial_yaw = float(sys.argv[4])

        rospy.init_node('wait_for_amcl_pose', anonymous=True)
        
        # 将初始位姿存储为全局变量供回调函数使用
        pose_params = {"x": initial_x, "y": initial_y, "yaw": initial_yaw}
