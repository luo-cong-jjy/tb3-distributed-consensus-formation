...
4. 区分静态障碍物和其他机器人
5. 计算并发布避障力信号
6. 根据雷达型号调整参数（雷达型号: LDS-01 或 LDS-02(默认)）
"""
...
class LaserObstacleAnalyzer:
    def __init__(self, robot_namespace="/tb3_0", lidar_type="LDS-01"):
        ...
        """
        # 注意：rospy.init_node() 已在main函数中调用
        ...
        rospy.loginfo(f"🔍 激光雷达障碍物分析器已启动 - {robot_namespace}/scan ({self.lidar_type})")
    ...
    def laser_callback(self, msg):
        ...
        if self.scan_count == 1:
            rospy.loginfo(f"🔍 {self.robot_namespace}: 雷达数据流已建立，开始障碍物分析")
        ...
        # 计算每个分区的长度并分类
        self.analyze_clusters(filtered_clusters, msg)
        ...
        # 显示性能统计（每50次扫描，减少输出频率）
        if self.scan_count % 50 == 0:
            avg_time = self.total_processing_time / self.scan_count
            rospy.loginfo(f"🚀 {self.robot_namespace}: 雷达聚类处理性能 - 平均{avg_time*1000:.1f}ms, 频率{1/avg_time:.1f}Hz")
    ...
    def cluster_by_distance_jump(self, points):
        ...
        # 改进的分割条件：
        # 1. 距离跳跃大于阈值（传统方法）
        # 2. 空间距离过大（在笛卡尔坐标系中的实际距离，同时覆盖了角度跳跃的情况）
        split_mask = (
            (distance_jumps >= self.distance_jump_threshold) |  # 距离跳跃
            (spatial_distances >= 0.08)  # 空间距离跳跃8cm（已包含角度跳跃效果）
        )
    ...
    def analyze_clusters(self, clusters, laser_msg, original_point_counts=None):
        """
        分析每个分区的长度并分类为静态障碍物或机器人，最后筛选出最近的障碍点
        
        Args:
            clusters: 分区后的点集列表
            laser_msg: 激光雷达消息（包含header用于时间戳同步）
        ...
        # 只在第1次扫描时输出关键信息
        if self.scan_count == 1:
            rospy.loginfo(f"🎯 {self.robot_namespace}: 障碍物检测 - 机器人:{len(robot_clusters)}, 静态:{len(static_clusters)}")
        # if self.scan_count <= 2:
...
if __name__ == '__main__':
    try:
        # 首先初始化节点
        rospy.init_node('laser_obstacle_analyzer', anonymous=True)
        
        # 然后从ROS参数获取配置
        robot_ns = rospy.get_param('~robot_namespace', 'tb3_0')
        lidar_type = rospy.get_param('~lidar_type', 'LDS-01')
        
        analyzer = LaserObstacleAnalyzer(robot_namespace=robot_ns, lidar_type=lidar_type)
        analyzer.run()
